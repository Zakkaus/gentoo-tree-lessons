...
- **Commit with `pkgdev commit`, not a raw `git commit`, so the subject follows the repository's `category/package: ...` convention; do not hand-write a divergent subject.** pkgdev derives the subject from the staged package and runs the repo checks; the autobump engine uses it, so every generated bump reads `cat/pkg: add X, drop Y`.
- **Land a new package (or one logical change) as a single clean commit; squash incremental fixups before the PR instead of a chain of `fix ...` / `use ...` / `update ...` follow-ups. Keep the history simple.** This is about the human's iteration: a bump/QA-fix that took several tries to get right (CI red, tweak, retry) must land as one squashed commit, not as the debugging trail. (An autobump engine's own output is already one plain `cat/pkg: add X, drop Y` commit — nothing to squash; this rule is for the hand-driven work around it.) Surfaced while iterating on the autobump/replay-eval tooling.
...
- **A haskell-cabal version bump is NOT a mechanical version-copy: hackport regenerates the RDEPEND bounds, the ghc/cabal floors and CABAL_HACKAGE_REVISION from the upstream `.cabal`, none of which is in the old ebuild. Copying the ebuild to a new version silently keeps stale bounds — regenerate with hackport (an autobump engine escalates these instead of bumping).** Evidence: `gentoo-replay-eval` ground-truth cross-check — a clean-room agent reproduced 0/12 metadata-driven edit-bumps offline because the data lives only in the upstream `.cabal`; autobump-rb now escalates on `inherit haskell-cabal` for exactly this reason (recall 0%→92% on that set).
