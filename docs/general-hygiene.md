...
- **git-r3 live ebuilds must not set the user variable `EGIT_CLONE_TYPE`, and must not set `EGIT_MIN_CLONE_TYPE=shallow` (its default): an ebuild cannot force a clone shallower than the user's `EGIT_CLONE_TYPE` (default `single`), so a "force shallow to save bandwidth" line is both a no-op and misleading. Drop it; only set `EGIT_MIN_CLONE_TYPE=single`/`single+tags`/`mirror` when the build genuinely needs one branch's history, tags, or all refs** - evidence: git-r3.eclass `: "${EGIT_CLONE_TYPE:=single}"` (L77) and `: "${EGIT_MIN_CLONE_TYPE:=shallow}"` (L94); gentoo-zh app-i18n/rime-ice-9999 abused `EGIT_CLONE_TYPE="shallow"` in-ebuild, then "fixed" it to the no-op `EGIT_MIN_CLONE_TYPE="shallow"` under a "Force shallow clone (depth=1)" comment the eclass cannot honor.
